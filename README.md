...
**`mawf advise [--task "<text>"] [--difficulty 1-5] [--json]`** scores every host against the task deterministically — capabilityFit (≤30), skillMatch (≤30; failed/pending MCPs and disabled plugins never match), modelFit (≤25), costFit (≤15) + a +8 stayBonus on the current host; switching is recommended only when the winner leads by ≥10 (hysteresis). On `switch` it pre-creates a host-aware `.mawf/handoff/<ts>-<from>-<to>.md` brief and prints the exact launch command — dsh's is `kill -9 $(lsof -ti tcp:3080) && dsh web` (a live old instance holds port 3080). That brief is the guidance-level handoff gate: inspect source/target host differences from MAW evidence, ask/grill only unresolved model/workflow decisions, and do **not** treat the switch as ready before the user responds. **Advise never executes anything**; the human runs the command.

**`mawf advise --pool [--task "<text>"] [--json]`** — stage-gated **plugin-pool judgment**: for the batch of project-level MCP servers / skills / plugins across hosts (declarative catalog `defaults/pool-catalog.json`; seeds: agent-browser, codebase-memory-mcp, codegraph — multi-host per their upstreams, dsh detect-only honest-gap) it emits `add / keep / remove / noop` verdicts with reasons plus **defensive procedures**: installs are check-then-act (never clobber existing assets), removals carry a per-artifact **no-residue checklist**, and exclusion groups keep code-knowledge-graph indexers mutually exclusive (codegraph and codebase-memory-mcp are never recommended together; a detected pair gets a consolidation verdict, a both-absent task recommends exactly one). Stages derive from graph gate batches / plan review points; ≥2 judgments per stage are recorded in `.mawf/runtime/pool-state.json`, and `mawf doctor --project` WARNs when a stage has fewer. **Advisory-only** — mawf never executes installs/removals; the pool's only write is its state file. Verdict knobs (`threshold` / `stayBonus` / `removeLookback`) are overridable in `.mawf/config.yaml` under `pool:`.

**Proactive injection (project scope only — global prompt files are never touched).** `init/plan/install/update/upgrade` write an idempotent managed block (≤26 lines, `<!-- mawf:cross-host-advise BEGIN/END -->`) into the project root `AGENTS.md` + `CLAUDE.md` — surfaces every supported host loads. The block tells any session agent to: re-run the stay/switch analysis at session start and on the first prompt of each day (UTC+8, freshness state in `.mawf/runtime/advise-state.json`); surface the recommendation with reasons; on switch, fill the handoff brief, inspect source/target host differences from MAW evidence, ask/grill only unresolved model/workflow decisions, and **not** treat the switch as ready before the user responds; pick up fresh (<48h) handoff briefs; consult the digest before claiming anything is "missing" on this machine; and at every stage boundary (graph gate batches / plan review points) run `mawf advise --pool` (≥2 judgments per stage), present the add/keep/remove verdicts to the human, and never execute installs/removals mid-batch. `mawf uninstall` keeps blocks by default; `--purge-config` strips them. Note (codex ≥0.150.0): untrusted projects ignore project-level `AGENTS.md` — grant project trust in codex once, or the block never loads in codex sessions (`mawf doctor` shows the reminder).
